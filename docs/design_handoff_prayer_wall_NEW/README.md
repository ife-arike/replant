...
- **(2026-07-25) Self-intercede stays blocked — Founder ruling.** Standing in the gap is on another's behalf; the count must mean OTHERS. Own-authored requests never show the Intercede button: the expanded action row reads `YOUR CHURCH'S REQUEST` (gate-notice register) with `{n} interceding` keeping its uniform bottom-right seat. Backed by an `is_own` flag on `get_prayer_wall` (migration 20260725150000, applied live; author-level, mirrors `stand_in_the_gap`'s guard; author id still never ships). The `self_interaction_blocked` toast remains as a backstop for pre-flag wire rows.
