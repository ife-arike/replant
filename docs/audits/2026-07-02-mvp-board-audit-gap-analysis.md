...
2. **KAN-157 — LEGAL. ~~privacy policy + ToS do not exist~~ CORRECTED 2026-07-02 (Founder):** the audit's finding was repo-scoped and stale against the cowork workstream — the **LEGAL role is ACTIVE in cowork**, an **interim privacy policy draft v0.2** and a **ToS scoping note v0.1** ("App Store ToS Deliverable", 16-section structure) exist at `/Users/ife/Documents/Claude/Projects/Replant/`, alongside bylaws/covenant/COI/Form-1023 drafts. Still true: nothing is published, the v0.2 policy (2026-05-13) predates the June systems and needs a v0.3 refresh against a fresh data inventory, ToS drafting is blocked on 3 Founder decisions (naming · acceptance-flow · scripture anchors), KAN-205 remains blocked behind this ticket, and the `scrub_church_pii()` technical anchor in the ticket is stale (actual mechanism: Day-30 tombstone scrub via `fn_hard_delete_expired_soft_deletes`). Workstream now tracked under epic KAN-301 (Compliance & Store Readiness); see KAN-157 c.16190.
