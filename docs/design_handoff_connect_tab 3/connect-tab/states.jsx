...
function TabBar({ active = 4, unread = 0, badgeEnabled = true }) {
  const tabs = [
  ...
  const showBadge = badgeEnabled && unread > 0;
        ...
        return (
          <div key={t.name} className={'tc-tab' + (isActive ? ' active' : '')}>
            <div className="tc-icon-wrap">
              ...
              {i === 4 && showBadge && (
                <span className="tab-badge" aria-label={unread + ' unread messages'}>{badgeText}</span>
              )}
            </div>
...
// ── Attachment “coming soon” popover (anchored above the composer paperclip) ─
function AttachPopover() {
  return (
    <div className="attach-pop" role="status">
      <div className="ap-title">Attachments — coming soon</div>
      <div className="ap-sub">Sharing files and photos is on the way — with consent, of course.</div>
...
Object.assign(window, {
  Icon, IconBranch, TabBar, AttachPopover, Monogram, BranchSeal, Segmented, CovenantFooter, CovenantStrip,
