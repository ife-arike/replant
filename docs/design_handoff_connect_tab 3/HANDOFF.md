...
- Enumerated (430 × 932):
  | Property | Value |
  |---|---|
  | Min-width × height | **17 × 17px** (grows for 2–3 digits via `padding: 0 4px`) |
  | Border-radius | **8.5px** |
  | Background | `--red` `#E05555` |
  | Text | `#FFFFFF`, DM Sans **10px / 600**, line-height 1 |
  | Separation ring | **1.5px solid `--bg` `#080808`** (box-sizing: border-box) |
  | Position | **pin to the icon's top-right corner with slight overlap** — `top: -5px; left: 9px` relative to the 20 × 20 icon. The badge should *overlap* the icon's upper-right, not float in the space above it. |
  | Tab icon / label | 20 × 20 icon · DM Sans 9.5px label |
- **Placement note (the fix):** anchor the badge to the icon's top-right corner so its lower-left
  overlaps the glyph. Earlier it sat too high/far-right and read as detached. Pin to the corner with a
  few px of overlap and a thin (1.5px) bar-colored ring for separation. On the rightmost tab, keep it
  pinned to the glyph — do **not** push it toward the screen edge.
- RN: use React Navigation's `tabBarBadge` (renders a native badge) styled to the above, fed by a
  `totalUnread` selector; hide at 0; respect the badge preference flag. Native iOS/Android badges
  already pin to the icon corner — if you use the platform badge, accept its default placement rather
  than offsetting it outward.
