...
function CalGlobe({ markerStyle, pulseSpeed, dimmed, onPickChurch, onPickRegion }) {
  ...
  const isRegional = zoom > 1.9;

  return (
    <div
      ref={wrapRef}
      className={'cal-globe-wrap' + (dimmed ? ' dimmed' : '') + (isRegional ? ' regional' : '')}
    ...
    >
      ...
      {/* zoom indicator (only visible when zoomed in) */}
      {zoom > 1.05 && (
        <div className="globe-zoom-pill">
          {isRegional ? 'Regional view' : `${Math.round(zoom * 100)}%`}
          {isRegional && (
