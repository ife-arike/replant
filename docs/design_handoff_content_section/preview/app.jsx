/* eslint-disable */
// ── Content Section · app shell + prototype harness ─────────────────
...
// deliverable screens (harness jump targets)
const SCREENS = {
  pattern:      { group: 'Pattern',       label: 'Shared pattern reference sheet', nav: null,            crumb: 'Content · Shared pattern', title: 'Content pattern' },
  'ann-home':   { group: 'Announcements', label: 'Home tab',                       nav: 'announcements', crumb: 'Content', title: 'Announcements', surface: 'ann', init: { topLevel: 'announcements', workflow: 'home' } },
  ...
  'ann-editor': { group: 'Announcements', label: 'Editor · 5 fields + preview',    nav: 'announcements', crumb: 'Content', title: 'Announcements', surface: 'ann', init: { view: 'editor', previewOpen: true } },
  'ann-witness':{ group: 'Announcements', label: 'Witness of the Day sibling',     nav: 'announcements', crumb: 'Content', title: 'Announcements', surface: 'ann', init: { topLevel: 'witness' } },
  ...
  'scr-home':   { group: 'Daily Scripture', label: 'Home tab',                     nav: 'scripture',     crumb: 'Content', title: 'Daily Scripture', surface: 'scr', init: { workflow: 'home' } },
  ...
  'scr-filter': { group: 'Daily Scripture', label: 'Filter · Theme/Translation/Book', nav: 'scripture',  crumb: 'Content', title: 'Daily Scripture', surface: 'scr', init: { workflow: 'home', drawer: 'filter' } },
  'out-home':   { group: 'Outreach · Ph.1', label: 'Home tab (curator)',          nav: 'outreach',      crumb: 'Content', title: 'Outreach & Missions', surface: 'out', init: { workflow: 'home' } },
  ...
  'out-leader': { group: 'Outreach · Ph.1', label: 'Leader mobile view',          nav: 'outreach',      crumb: 'Content', title: 'Outreach & Missions', surface: 'out', init: { view: 'leaderview' } },
...
function App() {
  ...
  function flash(msg, sub, sky) { setToast({ msg, sub, sky }); clearTimeout(window.__cst); window.__cst = setTimeout(() => setToast(null), 4200); }
  ...
  else if (sc.surface === 'ann') content = <AnnouncementsSurface key={screen} initial={sc.init} flash={flash} />;
  else if (sc.surface === 'scr') content = <ScriptureSurface key={screen} initial={sc.init} flash={flash} />;
  else if (sc.surface === 'out') content = <OutreachSurface key={screen} initial={sc.init} flash={flash} />;

  // group the harness jump list
  const groups = [];
      ...
      <div className="rp-main">
        <div className="rp-topbar">
          ...
          <div className="rp-top-meta">
            <span className="cs-viewer"><span className="vt-dot" /><span className="vt-name">{VIEWER[tier].first}</span><span className="vt-tier">{TIER_LABEL[tier]}</span></span>
          </div>
        </div>
        <div className="rp-body">{content}</div>
      </div>

      {toast && <Toast msg={toast.msg} sub={toast.sub} sky={toast.sky} />}
      ...
      <div className={`cs-harness ${harnessOpen ? '' : 'collapsed'}`}>
        ...
        <div className="cs-harness-body">
          <div className="cs-hfield">
            ...
            <div className="cs-seg tiers">
              {['regular', 'super_admin', 'top_tier'].map(t => (
                <button key={t} className={tier === t ? 'on' : ''} onClick={() => setTier(t)}>{TIER_LABEL[t]}</button>
              ))}
            </div>
            <span className="cs-hnote">Content is curated by <b>super_admin</b> curators. All three tiers can read the surface; Team Management + org approval gate above.</span>
          </div>
