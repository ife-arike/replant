...
// ── CAL regional grouping ────────────────────────────────────────────────
// Macro-regions for the "Regional view" on Church at Large. Every global
// church is assigned to the nearest region center (great-circle), so the
// buckets stay self-consistent as new churches are added — no per-church
// region field to maintain. The globe uses these centers to decide which
// region is facing the viewer.
const REGION_DEFS = [
...
function _greatCircle(aLat, aLon, bLat, bLon) {
...
function regionKeyForChurch(c) {
  let best = null, bestD = Infinity;
  for (const r of REGION_DEFS) {
    const d = _greatCircle(c.lat, c.lon, r.lat, r.lon);
    if (d < bestD) { bestD = d; best = r.key; }
  }
  return best;
}

// Regions with their churches grouped in. Only regions that actually hold a
// church are kept. `underground` flags regions the network acknowledges but
// cannot picture — drives the "gatherings we cannot name" footer.
const REGIONS = REGION_DEFS
  .map(r => ({
    ...r,
    churches: GLOBAL_CHURCHES.filter(c => regionKeyForChurch(c) === r.key),
    underground: COUNTS.underground_regions.includes(r.name),
  }))
...
Object.assign(window, {
  ...
  REGIONS, REGION_DEFS, regionKeyForChurch,
});
