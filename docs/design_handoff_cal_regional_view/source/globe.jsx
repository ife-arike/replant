...
function CalGlobe({ markerStyle, pulseSpeed, dimmed, onPickChurch, onPickRegion, onFaceRegion }) {
  const [lon0, setLon0] = React.useState(40);
  const [paused, setPaused] = React.useState(false);
  const [resuming, setResuming] = React.useState(false);
  const [zoom, setZoom] = React.useState(1);          // 1.0 default, up to 2.4
  ...
  React.useEffect(() => {
    function tick(t) {
      ...
      lastT.current = t;
      if (!paused) {
        setLon0(prev => (prev + dt * 0.006) % 360);
      }
  ...
  // helpers to manage pause/resume cycle
  const pauseRotation = () => {
    setPaused(true);
    setResuming(false);
  ...
  const scheduleResume = () => {
    ...
    // after 3.5s of stillness, show cue for 600ms, then resume
  ...
  const onPointerDown = (e) => {
    pauseRotation();
    if (e.target.closest('[data-globe-dot]')) return; // let dot click through
    dragState.current = {
      startX: e.clientX, startY: e.clientY,
      lon0Start: lon0, moved: false,
    };
  ...
  const onPointerMove = (e) => {
    ...
    setLon0((dragState.current.lon0Start - dx * 0.5 + 720) % 360);
  };
  const onPointerUp = (e) => {
    ...
    scheduleResume();
    // A tap on the globe body (no drag, not a dot) opens whichever region is
    // currently centered.
    if (wasDrag && !wasDrag.moved && faced) {
      onPickRegion && onPickRegion(faced);
    }
  };
  const onWheel = (e) => {
    // wheel = trackpad pinch on mac, or mouse wheel
    e.preventDefault();
    pauseRotation();
    setZoom(z => Math.max(1, Math.min(2.4, z * (e.deltaY < 0 ? 1.08 : 1/1.08))));
    scheduleResume();
  };
  ...
  const onTouchMove = (e) => {
    if (e.touches.length === 2 && pinchState.current) {
      ...
      setZoom(Math.max(1, Math.min(2.4, pinchState.current.startZoom * ratio)));
    }
  };
  const onTouchEnd = (e) => {
    if (e.touches.length < 2 && pinchState.current) {
  ...
  const isZoomed = zoom > 1.9;

  // Which macro-region is facing the viewer? In an orthographic globe that
  // only spins in longitude, the front-center is (lat 0, lon = lon0); the
  // region whose center projects with the largest z is the one we're looking
  // at. Recomputes as the globe rotates.
  const faced = React.useMemo(() => {
    const regions = window.REGIONS || [];
    let best = null, bestZ = -Infinity;
    for (const r of regions) {
      const p = project(r.lat, r.lon, lon0);
      if (p.z > bestZ) { bestZ = p.z; best = r; }
    }
    return best;
  }, [lon0]);

  // Notify the app when the faced region *changes* (not every rotation frame),
  // so the region pill always names whatever is currently centered.
  const facedKeyRef = React.useRef(null);
  React.useEffect(() => {
    if (faced && faced.key !== facedKeyRef.current) {
      facedKeyRef.current = faced.key;
      onFaceRegion && onFaceRegion(faced);
    }
  }, [faced, onFaceRegion]);
  ...
  return (
    <div
      ref={wrapRef}
      className={'cal-globe-wrap' + (dimmed ? ' dimmed' : '') + (isZoomed ? ' zoomed' : '')}
    ...
    >
      ...
      {visibleDots.map(d => {
        ...
        return (
          <div
            key={d.id}
            data-globe-dot
            onClick={(e) => { if (!dragState.current?.moved) onPickChurch(d); }}
            style={{
              position: 'absolute',
              left: left + '%',
              top: top + '%',
      ...
      {/* resume-rotation cue: subtle rotating arrow that fades in just before
          auto-rotation kicks back in after touch */}
      <div className={'globe-resume-cue' + (resuming ? ' show' : '')}>
        <svg width="22" height="22" viewBox="0 0 22 22">
          <path d="M 19 11 a 8 8 0 1 1 -2.5 -5.8" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
          <path d="M 19 5 L 19 11 L 13 11" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        <span>Resuming</span>
      </div>

      {/* zoom indicator — pure "look closer" magnifier readout */}
      {zoom > 1.05 && (
        <div className="globe-zoom-pill">
          {`${Math.round(zoom * 100)}%`}
          <span className="reset" onClick={(e) => { e.stopPropagation(); setZoom(1); pauseRotation(); scheduleResume(); }}>Reset</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className={'regional-panel' + (open ? ' open' : '')}>
      ...
      <div className="regional-body">
        ...
        {region.underground && (
          <div className="regional-hidden">
            <span className="d r" />
            + gatherings we cannot name
          </div>
        )}
      </div>
    </div>
